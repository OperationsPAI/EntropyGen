...
├── SOUL.md                         ← Agent 人格/角色定义（来自 Role ConfigMap > spec.soul）
├── AGENTS.md                       ← Agent 行为约束（来自 Role ConfigMap > 模板）
├── skills/                         ← OpenClaw Skills（内置 + Role 自定义合并）
│   ├── gitea-api/
│   │   └── SKILL.md                ← Gitea 操作技能
│   ├── git-ops/
│   │   └── SKILL.md                ← Git 操作技能
...
│   │   └── SKILL.md
│   └── {custom}/                   ← Role ConfigMap 中的自定义 skills
│       └── SKILL.md
├── {role-extra-files}              ← Role ConfigMap 中的非 well-known 文件
├── cron-config.json                ← Cron 配置（schedule + prompt）
...
| 路径 | 来源 | 生命周期 |
|------|------|---------|
| `/agent/config/*` → `~/.openclaw/` | ConfigMap `agent-{name}-config` | 随 CR spec 或 Role 变更更新 |
| `/agent/skills/*` → `~/.openclaw/skills/` | ConfigMap `agent-{name}-skills` | Skills 变更时更新 |
| `/agent/role/*` → `~/.openclaw/` | ConfigMap `agent-{name}-role-files`（可选） | Role 额外文件变更时更新 |
| `/agent/secrets/gitea-token` | Secret `agent-{name}-gitea-token` | 只读挂载 |
| `/agent/secrets/jwt-token` | Secret `agent-{name}-jwt-token` | 只读挂载 |
| `workspace/` | PVC `agent-{name}-workspace` | 跨重启持久化 |

> 注：ConfigMap 挂载到 `/agent/*` 为只读卷，`entrypoint.sh` 在启动时复制到可写的 `~/.openclaw/` 目录。
