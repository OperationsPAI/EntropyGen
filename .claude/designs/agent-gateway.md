...
- 不修改请求/响应内容（透明代理）
- per-Agent 令牌桶限流（默认 60 RPM，突发 10）；LLM 层额外限流由 LiteLLM 负责
- 不存储非 LLM 请求的 body（Gitea API / Git HTTP 请求不记录 body）
...
K8S Secret: agent-gateway-jwt-secret
  Namespace: aidevops
  Key: jwt-secret   ← 随机 256-bit hex 字符串
```
...
kubectl delete secret agent-developer-1-jwt-token -n aidevops

# 2. Operator 自动检测 Secret 缺失，在下次 Reconcile 时重新签发新 Token
# （Operator Watch aidevops namespace 的 Secret 变化，或通过定期 Reconcile 发现）
...
> **注**：`request_body` 和 `response_body` 仅在 `gateway.llm_inference` 事件中写入（其他事件类型此字段为空字符串）。单个 body 超过 64KB 时截断，并设置 `_body_truncated: true`。

`event_type` 枚举：

| `event_type` | 触发条件 | 携带 body |
|-------------|---------|---------|
| `gateway.llm_inference` | 转发到 LiteLLM 的请求 | **是**（request_body + response_body） |
| `gateway.gitea_api` | 转发到 Gitea REST API 的请求 | 否 |
| `gateway.git_http` | 转发到 Gitea Git HTTP 的请求 | 否 |
| `gateway.heartbeat` | Agent 心跳请求（特殊 path `/heartbeat`） | 否 |
| `gateway.auth_failed` | Token 验证失败 | 否 |

> **LLM body 记录策略**：仅 `gateway.llm_inference` 事件写入 `request_body` 和 `response_body`（OpenAI format JSON 字符串），用于训练数据导出。单个 body 超过 64KB 时截断并标记 `_body_truncated: true`。非 LLM 请求（Gitea API、Git HTTP）不记录 body，避免 Redis 内存压力。
...
# Gateway ConfigMap
upstream:
  litellm: "http://litellm.aidevops.svc:4000"
  gitea: "http://gitea.aidevops.svc:3000"

redis:
  addr: "redis.aidevops.svc:6379"
...
流量通过 K8S Service（ClusterIP）负载均衡，Agent 连接 `http://agent-gateway.aidevops.svc`，无需感知副本数量。
...
仅记录 LLM 推理请求（`gateway.llm_inference`）的 body，用于训练数据导出。

**记录内容：**

| 字段 | 内容 | 大小限制 |
|------|------|---------|
| `request_body` | OpenAI `/v1/chat/completions` 请求体（含 `messages` 数组） | 最大 64KB |
| `response_body` | LiteLLM 返回的 OpenAI 格式响应体（含 `choices` 数组） | 最大 64KB |

超出 64KB 时：截断 JSON 字符串末尾，追加 `"...[TRUNCATED]"`，并在 payload 中设置 `_body_truncated: true`。

**非 LLM 请求不记录 body：**
- `gateway.gitea_api`：只记录 method、path、status_code、latency_ms
- `gateway.git_http`：只记录 method、path、status_code、size_bytes
- `gateway.heartbeat`：只记录 status

**Redis 内存影响估算：**

```
每条 LLM 事件 body 平均大小：~8KB（request ~6KB + response ~2KB）
每天 LLM 推理次数（5 Agent）：~2000 次
events:gateway MAXLEN ~ 100000 条（含所有类型）
实际占用：~16MB（LLM body 部分）+ ~5MB（元数据部分）≈ 21MB
```

Redis 512MB 配置下，LLM body 记录对总内存占用影响可接受。
