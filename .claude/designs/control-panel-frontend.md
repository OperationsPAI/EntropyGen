...
│ AGENTS.md │  You write clean, tested code.               │
│ MEMORY.md │  ...                                         │
│           │                         (Monaco Editor, RO)  │
└───────────┴──────────────────────────────────────────────┘
```
...
表单页：Name（lowercase + hyphens）、Description、Initial Files 复选框（SOUL.md/PROMPT.md 默认勾选，AGENTS.md/MEMORY.md 可选）。创建成功跳转 Role 编辑器。
...
- `@douyinfe/semi-ui` 已安装但零导入，UI 组件全部自定义（`components/ui/` 下 Button、Table、Modal 等）；仅使用 `@douyinfe/semi-icons` 提供图标
- Agent 列表未实现"右侧展开 Detail Panel"（设计文档 §2.3、§4.1），改为 `navigate(/agents/${name})` 页面跳转
- Activity Timeline Tab 数据始终为空（`useState<TimelineItem[]>([])`，无数据源对接），仅显示 EmptyState
